@php
    ...
    $languages = \Webkernel\Applications\Users\Models\Lang\Language::getActiveLanguages();
    $currentLang = auth()->check() ? auth()->user()->user_lang : (session('locale') ?? config('app.locale'));
    $currentLangLabel = $languages && $languages->count() > 0 ? optional($languages->firstWhere('code', $currentLang))->label ?? $currentLang : $currentLang;
    ...
    $path = base_path('packages/webkernel/src/Applications/UI/public/assets/flags/language/' . $currentLang . '.svg');
    ...
    $languageSVG = function($languageCode) use ($processSvg) {
        $path = base_path( 'packages/webkernel/src/Applications/UI/public/assets/flags/language/' . $languageCode . '.svg');
        $svgContent = file_exists($path) ? file_get_contents($path) : '';
