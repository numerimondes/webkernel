<style>
    ...
    .submenu-wrapper.full .submenu-content__list[data-layout="grid-3"],
    .submenu-content__list[data-layout="grid-3"] {
        display: grid;
        grid-template-columns: repeat(3, minmax(0, 1fr));
        gap: 24px;
    }

    .submenu-wrapper.full .submenu-content__list[data-layout="grid-2-detail"],
    .submenu-content__list[data-layout="grid-2-detail"] {
        display: grid;
    ...
    .submenu-content__list[data-layout="grid-2-detail"] .submenu-content__link-img {
        width: 100%;
    ...
    .submenu-content__list[data-layout="grid-2-detail"] .submenu-content__link-title,
    .submenu-content__list[data-layout="grid-2-detail"] .submenu-content__link-text {
        padding: 0;
        margin-bottom: 12px;
    }
    ...
    .submenu-content__list[data-variant="cards"] .submenu-content__link {
    ...
    .submenu-content__list[data-variant="details"] .submenu-content__link {
...
@php
    $defaultMenuTypeConfig = [
        'title' => 'Items',
        'layout' => 'grid-3', // grid-3 | grid-2-detail
        'variant' => 'cards', // cards | details
    ];
    ...
    $menuTypeConfigs = [
        ...
        'opportunities' => [
            'title' => 'Future Events',
            'layout' => 'grid-2-detail',
            'variant' => 'details',
        ],
    ];

    // Defaults and normalizers for flexible classes and future DB input
    $menuDefaults = [
        'classes' => [
            'li' => '',
            'link' => '',
    ...
    $mergeClasses = function (array $provided = null, array $defaults = []) {
        return array_merge($defaults, $provided ?? []);
    };

    $normalizeOtherLink = function ($link) use ($mergeClasses) {
        if (is_string($link)) {
            return [
                'label' => $link,
                'href' => '#',
                'target' => null,
                'classes' => $mergeClasses([], ['li' => '', 'link' => '']),
            ];
        }
        $href = $link['href'] ?? (isset($link['route']) ? route($link['route'], $link['params'] ?? []) : '#');
        return [
            'label' => $link['label'] ?? ($link['title'] ?? ''),
            'href' => $href,
            'target' => $link['target'] ?? null,
            'classes' => $mergeClasses($link['classes'] ?? [], ['li' => '', 'link' => '']),
        ];
    };

    $normalizeMenu = function (array $menu) use ($menuDefaults, $menuTypeConfigs, $defaultMenuTypeConfig, $mergeClasses) {
        $cfg = $menuTypeConfigs[$menu['type'] ?? ''] ?? $defaultMenuTypeConfig;
        $classes = $mergeClasses($menu['classes'] ?? [], $menuDefaults['classes']);
        $fullThreshold = $menu['full_threshold'] ?? $menuDefaults['full_threshold'];
        $categories = array_map(function ($cat) use ($classes, $mergeClasses) {
            $cat['classes'] = $mergeClasses($cat['classes'] ?? [], ['li' => '', 'link' => '']);
            return $cat;
        }, $menu['categories'] ?? []);
        return [
            'title' => $menu['title'] ?? '',
            'type' => $menu['type'] ?? 'products',
            'cfg' => $cfg,
            'classes' => $classes,
    ...
    // In production, this will come from DB; normalize now to support DB shape
    $menus = [
        [
            'title' => 'Products',
            'type' => 'products',
    ...
    [
        'label'   => 'Shop',
        'href'    => Route::has('shop.index') ? route('shop.index', [], false) : '/',
        'classes' => ['li' => '', 'link' => '']
    ],
    [
        'label'   => 'Fondateur',
        'href'    => Route::has('founder.show') ? route('founder.show', [], false) : '/',
        'classes' => ['li' => '', 'link' => '']
    ],
    [
        'label'   => 'Expertise',
        'href'    => Route::has('expertise.index') ? route('expertise.index', [], false) : '/',
        'classes' => ['li' => '', 'link' => '']
    ],
    [
        'label'   => 'Contact',
        'href'    => Route::has('contact.index') ? route('contact.index', [], false) : '/',
        'classes' => ['li' => '', 'link' => '']
    ],
];
@endphp
                    ...
                    <div class="header__navigation-wrapper">
                        <nav class="header__navigation">
                            <ul class="header__list">
                                @php
                                    // Prepare data for rendering: normalize menu entries (future DB compatibility)
                                    $normalizedMenus = array_map($normalizeMenu, $menus);
                                @endphp
                                @foreach ($normalizedMenus as $menu)
                                    <li class="header__list-item has-submenu {{ $menu['classes']['li'] }}">
                                        <a href="#" class="{{ $menu['classes']['link'] }} inform-user-how-to-pin">
                                        ...
                                        <div class="submenu-wrapper {{ $menu['classes']['submenuWrapper'] }}" data-menu-type="{{ $menu['type'] }}" @if($menu['full_threshold']) data-full-threshold="{{ $menu['full_threshold'] }}" @endif>
                                            <div class="submenu-list__wrapper">
                                                <div class="submenu-list__title">Categories</div>
                                                <ul class="submenu-list">
                                                    @foreach ($menu['categories'] as $index => $category)
                                                        <li class="submenu-list__item has-submenu {{ $index === 0 ? 'active' : '' }} {{ $category['classes']['li'] ?? '' }}">
                                                            <div class="submenu-list__item-wrapper">
                                                                <div class="submenu-list__item-icon">
                                                                    {!! $category['icon'] !!}
                                                                </div>
                                                                <a href="#" class="submenu-list__item-link {{ $category['classes']['link'] ?? '' }}">
                                                            ...
                                                            <div class="submenu-content">
                                                                <div class="submenu-content__title">{{ $menu['cfg']['title'] }}</div>
                                                                <ul class="submenu-content__list {{ $menu['classes']['contentList'] }}" data-layout="{{ $menu['cfg']['layout'] }}" data-variant="{{ $menu['cfg']['variant'] }}">
                                                                    @foreach (($category['items'] ?? []) as $item)
                                                                        <li class="submenu-content__list-item">
                                                                            @if (($menu['cfg']['variant'] ?? 'cards') === 'cards')
                                ...
                                @endforeach
                                @php $normalizedOtherLinks = array_map($normalizeOtherLink, $otherLinks); @endphp
                                @foreach ($normalizedOtherLinks as $link)
                                    <li class="header__list-item {{ $link['classes']['li'] }}">
                                        <a href="{{ $link['href'] }}" @if($link['target']) target="{{ $link['target'] }}" rel="noopener" @endif class="{{ $link['classes']['link'] }}">{{ $link['label'] }}</a>
                                    </li>
                                @endforeach
                            </ul>
                        </nav>
...
<script>
    window.addEventListener("DOMContentLoaded", () => {
        ...
        // Lazy render submenu contents: detach on load, mount/unmount when needed
        const detachContentForItem = (li) => {
            const contentEl = li.querySelector('.submenu-content');
            if (contentEl) {
                li.__submenuContent = contentEl;
                contentEl.remove();
            }
        };

        const mountContentForItem = (li) => {
            if (!li.querySelector('.submenu-content') && li.__submenuContent) {
                li.appendChild(li.__submenuContent);
            }
        };

        const unmountContentForItem = (li) => {
            const contentEl = li.querySelector('.submenu-content');
            if (contentEl) {
                li.__submenuContent = contentEl;
                contentEl.remove();
        ...
        const mountActiveSubcategory = (topItem) => {
            const activeLi = topItem.querySelector('.submenu-list__item.has-submenu.active');
            if (activeLi) {
                mountContentForItem(activeLi);
            }
        };

        // Detach all submenu contents initially so hidden items are not present in the DOM
        document.querySelectorAll('.submenu-list__item.has-submenu').forEach(detachContentForItem);
            ...
            const scheduleClose = () => {
                ...
                closeTimeout = setTimeout(() => {
                    if (!item.classList.contains("pinned")) {
                        // Unmount all submenu contents for this top-level item
                        item.querySelectorAll('.submenu-list__item.has-submenu').forEach(unmountContentForItem);
                ...
                item.addEventListener("click", (e) => {
                    if (window.innerWidth > 1025) {
                        ...
                        submenuItems.forEach((i) => {
                            ...
                            unmountContentForItem(i);
                        });
                        item.classList.add("active");
                        mountContentForItem(item);
                    }
                });
            });
    ...
    window.addEventListener("DOMContentLoaded", () => {
        ...
        document.querySelectorAll('.submenu-wrapper').forEach(wrapper => {
            ...
            const threshold = parseInt(wrapper.dataset.fullThreshold || '500', 10);
        ...
        const closeAllMenus = () => {
            document
                ...
                .forEach(i => {
                    i.querySelectorAll('.submenu-list__item.has-submenu').forEach(li => {
                        const c = li.querySelector('.submenu-content');
                        if (c) c.remove();
                    });
                    i.classList.remove('open','pinned');
                });
            document.body.classList.toggle('mm-open', !!document.querySelector('.header__list-item.open'));
            if (typeof layoutOpenMegaMenus === 'function') layoutOpenMegaMenus();
        };
...
<script>
    document.querySelectorAll('.inform-user-how-to-pin').forEach(el => {
        const tooltipText = 'Appuyez pour épingler le menu';
