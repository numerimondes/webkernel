...
@push('styles')
<link rel="preload" as="image" href="{{ $backgroundImageLightMode }}">
<link rel="preload" as="image" href="{{ $backgroundImageDarkMode }}">
<style>
    :root {
        ...
        --z-card: 1;
        --z-content: 10;
        --simple-main-max-width: {{ $cardMaxWidth }};
        --bg-image-light: url('{{ $backgroundImageLightMode }}');
        --bg-image-dark: url('{{ $backgroundImageDarkMode }}');
    }
    ...
    body::after {
        ...
        opacity: 0;
        transition: opacity {{ $transitionDuration }} ease-in-out;
        filter: brightness({{ $darkOverlayOpacity }});
    }
    ...
    .slide-up {
        animation: slideUp {{ $fadeInDuration }} ease-out;
    }
