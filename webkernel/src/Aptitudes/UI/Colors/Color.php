...
class Color
{
    ...
    /**
     * Create a custom color palette from any color format (hex, rgb, oklch)
     */
    public static function custom(string $color, string $name = 'custom'): array
    {
        return static::generatePalette($color);
    }

    /**
     * Validate and normalize color input from user
     */
    public static function normalizeColor(string $color): string
    {
        $color = trim($color);

        // If it's already OKLCH, return as is
        if (str_starts_with($color, 'oklch(')) {
            return $color;
        }

        // If it's hex, convert to OKLCH
        if (str_starts_with($color, '#')) {
            return static::convertToOklch($color);
        }

        // If it's rgb, convert to OKLCH
        if (str_starts_with($color, 'rgb(') || str_starts_with($color, 'rgba(')) {
            return static::convertToOklch($color);
        }

        // If it's hsl, convert to OKLCH (basic conversion)
        if (str_starts_with($color, 'hsl(') || str_starts_with($color, 'hsla(')) {
            return static::convertHslToOklch($color);
        }

        // Try to parse as comma-separated RGB values
        if (preg_match('/^(\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3})$/', $color, $matches)) {
            $rgb = "rgb({$matches[1]}, {$matches[2]}, {$matches[3]})";
            return static::convertToOklch($rgb);
        }
    ...
    /**
     * Convert HSL to OKLCH (basic implementation)
     */
    protected static function convertHslToOklch(string $hsl): string
    {
        // Remove hsl( and ) from the string
        $hsl = str_replace(['hsl(', 'hsla(', ')'], '', $hsl);

        // Split by comma and clean
        $values = array_map('trim', explode(',', $hsl));

        if (count($values) < 3) {
            return $hsl; // Return original if invalid
        }
        ...
        $rgb = "rgb(" . round($r * 255) . ", " . round($g * 255) . ", " . round($b * 255) . ")";
        return static::convertToOklch($rgb);
    }

    /**
     * Get color palette by name or create custom palette
     */
    public static function getPalette(string $name): array
    {
        $allColors = static::all();

        // If it's a predefined color, return it
        if (isset($allColors[$name])) {
            return $allColors[$name];
        }

        // If it's a custom color, generate palette
        $normalizedColor = static::normalizeColor($name);
        return static::generatePalette($normalizedColor);
    }

    /**
     * Check if a color name is predefined
     */
    public static function isPredefinedColor(string $name): bool
    {
