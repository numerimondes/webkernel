...
<section
    ...
    x-data="{
        isCollapsed: @if ($persistCollapsed)
            (() => {
                const key = `section-${@js($collapseId) ?? $el.id}-isCollapsed`;
                return localStorage.getItem(key) ? JSON.parse(localStorage.getItem(key)) : @js($collapsed);
            })()
        @else @js($collapsed) @endif,
