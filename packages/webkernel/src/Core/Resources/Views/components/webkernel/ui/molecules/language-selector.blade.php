@php
    ...
    $languages = \Webkernel\Core\Models\Language::getActiveLanguages();
    ...
    $path = base_path(WEBKERNEL_CORE_PATH . '/public/assets/flags/language/' . $currentLang . '.svg');
    ...
    $languageSVG = function($languageCode) use ($processSvg) {
        $path = base_path(WEBKERNEL_CORE_PATH . '/public/assets/flags/language/' . $languageCode . '.svg');
        $svgContent = file_exists($path) ? file_get_contents($path) : '';
