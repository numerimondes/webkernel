...
determine_commit_type() {
    ...
    if echo "$files" | grep -qiE "(fix|bug|patch|hotfix)" || \
       echo "$diff_content" | grep -qiE "(fix|bug|error|issue|exception)"; then
        type="fix"
    # Check for new features vs refactoring based on addition/deletion ratio
...
ask_for_individual_commits() {
    local files="$1"
    local file_count=$(echo "$files" | wc -l)
    
    if [[ "$file_count" -gt 1 ]]; then
        echo ""
        echo -e "${CYAN}Would you like to commit files individually? (y/N):${NC}"
        read -r individual_choice
        if [[ "$individual_choice" =~ ^[Yy]$ ]]; then
            return 0
        fi
    fi
    return 1
...
commit_individual_files() {
    ...
    echo "$files" | while IFS= read -r file; do
        ...
        # Determine commit type and message for this file
...
gacp() {
    ...
    if [[ -z "$files" ]]; then
        echo -e "${YELLOW}[gacp] No staged changes to commit${NC}"
    ...
    # Ask for individual commits if multiple files
    if ask_for_individual_commits "$files"; then
...
if [[ -n "${BASH_VERSION:-}" ]]; then
    log_info "Running in bash"
elif [[ -n "${ZSH_VERSION:-}" ]]; then
    log_info "Running in zsh"
else
