...
class TranslateTerm extends Command
{
    ...
    protected $languageMap = [
        'ar' => 'ar', 'az' => 'az', 'bg' => 'bg', 'bn' => 'bn', 'bs' => 'bs', 'ca' => 'ca',
    ...
    protected function handleAddTranslation()
    {
        ...
        // webkernel is already set as default
    ...
    protected function handleChangeKey()
    {
        ...
        // Interactive input if missing
        while (empty($oldKey)) {
            $oldKey = $this->ask('Enter current key to change');
        }

        while (empty($newKey)) {
            $newKey = $this->ask('Enter new key');
        }
    ...
    protected function showAvailableKeys()
    {
        ...
        if (class_exists('Webkernel\Models\LanguageTranslation')) {
            try {
                $dbKeys = \Webkernel\Models\LanguageTranslation::distinct('lang_ref')->pluck('lang_ref')->toArray();
        ...
        if (!empty($allKeys)) {
            ...
            if (count($allKeys) > 10) {
                $this->line("  ... and " . (count($allKeys) - 10) . " more");
            }
        } else {
    ...
    protected function writeTranslation($locale, $term, $translation, $direction)
    {
        ...
        foreach ($directories as $dir) {
            ...
            if (!file_exists($filePath)) {
                // Exemple pour clé "hello" en français:
                // 'direction' => 'ltr',
                // 'actions' => [
                //     'hello' => [
                //         'label' => 'Bonjour',
                ...
                // ]
                //
                // Exemple pour clé "hello" en anglais:
                // 'direction' => 'ltr',
                // 'actions' => [
                //     'hello' => [
                //         'label' => 'Hello',
