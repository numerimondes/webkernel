@php
    ...
    // Appliquer les classes CSS au fichier SVG si disponible
    if ($svgContent) {
        // Ajouter dynamiquement les classes à l'élément SVG
        $svgContent = preg_replace('/<svg(.*?)>/', '<svg$1 class="h-4 w-4 object-contain">', $svgContent);
    }

    // Pour réutiliser dans le dropdown list
...
<div x-data="{
    ...
    <x-filament::dropdown maxHeight="250px" teleport placement="bottom-start">
        <x-slot name="trigger">
            <div class="fi-dropdown-list p-1">
                <div class="flex items-center justify-center h-8 w-8 rounded-md transition-colors hover:bg-gray-50 dark:hover:bg-white/5">
                    <!-- WebKernel Language Provider par www.numerimondes.com -->
                    {!! $svgContent !!}
                </div>
            </div>
        </x-slot>
        ...
        <x-filament::dropdown.list class="w-40">
            @foreach ($languages as $language)
                <x-filament::dropdown.list.item
                    x-bind:class="selectedLang === '{{ $language->code }}' ? 'bg-gray-100 dark:bg-gray-700' : ''"
