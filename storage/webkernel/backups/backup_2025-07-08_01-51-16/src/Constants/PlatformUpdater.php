...
class PlatformUpdater
{
    ...
    private function createBackup(): string
    {
        ...
        // Handle null result from shell_exec
        if ($result === null) {
            throw new Exception("Backup failed: shell_exec returned null");
        }
    ...
    private function downloadAndInstallUpdate(): bool
    {
        ...
        if (!$this->commandExists('git')) {
            throw new Exception("Git is not available on this system. Please install Git and try again.");
        }
        
        // Check if we have write permissions
        if (!is_writable($this->getBasePath())) {
            throw new Exception("No write permissions in: " . $this->getBasePath());
        }
        ...
        try {
            ...
            foreach ($commands as $cmd) {
                ...
                // Handle null result from shell_exec
                if ($result === null) {
                    throw new Exception("Git command failed: shell_exec returned null for command: {$cmd}");
                }
                
                // Log the result for debugging
                $this->log("Command result: " . substr($result, 0, 200) . (strlen($result) > 200 ? '...' : ''));
