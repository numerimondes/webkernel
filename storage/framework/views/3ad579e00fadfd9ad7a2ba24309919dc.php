<?php if(app()->environment('local') && (!isset($hideDebug) || !$hideDebug)): ?>
    <div class="fixed bottom-4 left-4 z-50 text-xs bg-black/75 text-white px-3 py-2 rounded-lg font-mono max-w-sm"
         x-data="{
            ...
            stats: {
                ...
                memoryUsage: '<?php echo e(round(memory_get_usage(true) / 1024 / 1024, 2)); ?> MB',
                peakMemory: '<?php echo e(round(memory_get_peak_usage(true) / 1024 / 1024, 2)); ?> MB'
            }
         }"
         x-init="
        ...
        <div x-show="expanded" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 transform -translate-y-2" x-transition:enter-end="opacity-100 transform translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 transform translate-y-0" x-transition:leave-end="opacity-0 transform -translate-y-2" class="mt-3 space-y-1 border-t border-gray-600 pt-2">
            <div class="grid grid-cols-2 gap-2">
                <div>
                    ...
                    <div class="text-green-400"><?php echo e(app()->environment()); ?></div>
                </div>
                <div>
                    <div class="text-gray-400">Laravel:</div>
                    <div><?php echo e(app()->version()); ?></div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-2">
                ...
                <div>
                    ...
                    <div><?php echo e(count(DB::getQueryLog())); ?></div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="text-xs break-all"><?php echo e(request()->route() ? request()->route()->getName() ?? request()->path() : request()->path()); ?></div>
            </div>

            <?php if(auth()->check()): ?>
                <div>
                    <div class="text-gray-400">User:</div>
                    <div class="text-xs"><?php echo e(auth()->user()->email ?? auth()->id()); ?></div>
                </div>
            <?php endif; ?>
    ...
    <script>
        ...
        if (typeof console !== 'undefined') {
            ...
            console.log('%cEnvironment:', 'font-weight: bold;', '<?php echo e(app()->environment()); ?>');
            console.log('%cLaravel Version:', 'font-weight: bold;', '<?php echo e(app()->version()); ?>');
            console.log('%cPHP Version:', 'font-weight: bold;', '<?php echo e(phpversion()); ?>');
            console.log('%cRoute:', 'font-weight: bold;', '<?php echo e(request()->route() ? request()->route()->getName() ?? request()->path() : request()->path()); ?>');
            <?php if(auth()->check()): ?>
                console.log('%cAuthenticated User:', 'font-weight: bold;', <?php echo e(json_encode(['id' => auth()->id(), 'email' => auth()->user()->email ?? 'N/A'])); ?>);
            <?php endif; ?>
            console.log('%cMemory Usage:', 'font-weight: bold;', '<?php echo e(round(memory_get_usage(true) / 1024 / 1024, 2)); ?> MB');
            console.log('%cDatabase Queries:', 'font-weight: bold;', <?php echo e(count(DB::getQueryLog())); ?>);
            console.groupEnd();
        }
...
<?php endif; ?>
<?php /**PATH /home/yassine/Documents/project/numerimondes-com/webkernel/src/Aptitudes/WebsiteBuilder/Resources/Views/live-website/layouts/partials/debug.blade.php ENDPATH**/ ?>